...
const sta2 = sta(limit());
console.log('DATA CHECK',sta);
console.log('Mapped data', mapper(sta, limit())[0][0]);
console.log('COMPOSITION', compose(limit, sta)(limit()[0]));

console.log('DATA', mapper(sta, limit())[0]);
...
const tdDisplay = (data) => data.map(x => tbl.innerHTML += `<tr><td>${x.name}</td><td>${createDateFormat(x.date).date}</td><td>${createDateFormat(x.date).hours}:${createDateFormat(x.date).minutes}${createDateFormat(x.date).ampm}</td><td>${x.uploads}</td><td>${x.downloads}</td></tr>`);



function displayPage (cont, e) {
	// console.log(sta2[e.target.value]);
	// console.log(e.target.nodeName);
	// console.log(e.target.value);
	 console.log('ATTRIBUTE', cont.getAttribute('data-val'));

	console.info('Is this right?', curry(splitData)(dataToDisplay)(limit()[e.target.value]));
	
	const elToRemove = document.getElementsByTagName('tbody');
	//console.log(elToRemove);
	//console.log(table);
	//tbl.remove(document.getElementsByTagName('tbody'));
	container.innerHTML = '';
	tbl.innerHTML = '';
	//appendElement(tbl, container);
	tdDisplay(mapper(sta, limit())[e.target.value][0]);
//   console.info('Is this right?', curry(splitData)(dataToDisplay)(limit()[e.target.value])[e.target.value]);
appendElement(ul, container);
appendElement(tr, thead);
appendElement(thead, tbl);
appendElement(tbl, container);
pagination(mapper(sta, limit())[e.target.value]);
};

tdDisplay(mapper(sta, limit())[0][0]);

const pagination = (ar) => {
	return ar.forEach((x, i) => {
		container.innerHTML += `<button value="${i}" onclick="renderPage(this, event)">${i + 1}</button>`;
	});
}

const renderPage = (data, e) => {
...
appendElement(thead, tbl);
appendElement(tbl, container);
pagination(mapper(sta, limit())[0]);

//console.log('what is it?',curry(splitData)(dataToDisplay)(limit()[0])[0]);
